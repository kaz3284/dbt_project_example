...
└── snowflake_setup/ # Snowflake側の環境構築・ロール・タグ・マスキングポリシー定義（事前に一度適用する管理者作業。詳細は snowflake_setup/README.md 参照）
    ├── init/         # ウェアハウス・データベース・DEVELOPERロール・キーペア認証の初回構築（先に実行）
    ├── roles/        # マスキングクリアランスロールの作成
    └── tags/         # タグ・マスキングポリシーの作成とアタッチ（対象データベースごとに実行）
```
...
- Snowflake側で事前に用意しておく必要がある環境構築・ロール・タグ・マスキングポリシーのSQLは
  [`snowflake_setup/README.md`](./snowflake_setup/README.md) を参照してください。
  `dbt run` の前に、`snowflake_setup/init/` → `snowflake_setup/roles/` → `snowflake_setup/tags/` の順で
  一度適用しないと、タグ設定用のpost-hookが `Schema 'XXXX_DB.TAGS' does not exist` のようなエラーで失敗します。
